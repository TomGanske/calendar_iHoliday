 ...
 */
class ModuleEventlistIHoliday extends \Events
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_iHoliDay_eventlist';
	...
	protected function compile()
	{
		...
		for ($i=$offset; $i<$limit; $i++)
		{
			...
			// Add Custom Style CSS Code
			$colorString = $this->getCssColor($event['startDate'],$event['endDate'],$this->progress_active,$this->progress_future);
			$objTemplate->progress_color = $colorString;
	 ...
	 * @return string
	 */
	public function getCssColor($startDate,$endDate,$activeColor,$futureColor)
	{
		# is $endDate = NULL set $endDate = $startDate
        if($endDate == NULL)
            $endDate = $startDate;

        // only Appointments for now and the future depends on settings
        // inside the modul in Contao Backend
		if($startDate < time() && $endDate <= time())
			return '#'.$activeColor;
		elseif($startDate > time() && $endDate > time())
			return '#'.$futureColor;
        else
            return '';
	}
